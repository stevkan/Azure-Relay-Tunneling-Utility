...

{
  internal class DispatcherService
  {
    ...
    public DispatcherService(RelayConfig config, RelayResourceManager resourceManager = null)
    {
      ...
      var relayFqdn = config.RelayNamespace.Contains(".servicebus.windows.net")
          ? config.RelayNamespace
          : $"{config.RelayNamespace}.servicebus.windows.net";
      _listener = new HybridConnectionListener(new Uri($"sb://{relayFqdn}/{config.RelayName}"), tokenProvider);
    ...
    public async Task OpenAsync(CancellationToken cancelToken)
    {
      ...
      {
        Console.WriteLine($"Creating Hybrid Connection '{_config.RelayName}'...");
      ...
      // Extract FQDN from listener address (sb://namespace.servicebus.windows.net/relay-name)
      var listenerUri = _listener.Address;
      var httpsEndpoint = $"https://{listenerUri.Host}/{listenerUri.AbsolutePath.TrimStart('/')}";

      Console.WriteLine("─────────────────────────────────────────────────────────");
      Console.WriteLine($"✓ Relay '{_config.RelayName}' is ready");
      Console.WriteLine($"  Public Endpoint:  {httpsEndpoint}");
      Console.WriteLine($"  Routing To:       {_httpClient.BaseAddress}");

      if (_config.EnableWebSocketSupport && _webSocketForwarder != null)
      {
        Console.WriteLine($"  WebSocket:        Enabled → {_targetWebSocketAddress}");
    ...
    public async Task CloseAsync(CancellationToken cancelToken)
    {
      ...
      if (_config.DynamicResourceCreation && _resourceManager != null)
      {
        ...
        if (!deleted)
        {
          Console.WriteLine($"⚠ Failed to delete '{_config.RelayName}'");
        }
      }
    }
